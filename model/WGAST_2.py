...
# ========================================================================================
# CHANGE 1: ADDED THE NEW DENOISINGRESBLOCK CLASS DEFINITION
# This is our new, learnable denoising module. It will be trained to
# intelligently remove artifacts from the generated LST map.
# ========================================================================================
class DenoisingResBlock(nn.Module):
    """
    A learnable residual block designed to replace a static filter.
    
    Instead of a fixed blur, this block learns to identify and model the
    unwanted artifacts (the "residual") in the generated image. By adding
    this learned residual back to the input, it effectively performs a
    data-driven denoising operation.
    """
    def __init__(self, channels=1):
        super(DenoisingResBlock, self).__init__()
        
        # A simple sequence of two convolutional layers with a ReLU activation.
        # This is where the "learning" happens.
        self.conv_block = nn.Sequential(
            # First convolution: expands to 32 feature maps to learn complex patterns.
            nn.Conv2d(in_channels=channels, out_channels=32, kernel_size=3, padding=1),
            nn.ReLU(inplace=True),
            # Second convolution: collapses back to the original number of channels.
            nn.Conv2d(in_channels=32, out_channels=channels, kernel_size=3, padding=1)
        )

    def forward(self, x):
        # 1. The block processes the input image to find the noise/artifacts.
        residual = self.conv_block(x)
        
        # 2. The learned residual is added back to the original input image.
        # This is the key step: it corrects the image by adding the learned "fix".
        return x + residual
# ========================================================================================
# END OF CHANGE 1
# ========================================================================================
...
class CombinFeatureGenerator(nn.Module):
    def __init__(self, NUM_BANDS=NUM_BANDS, ifAdaIN=True, ifAttention=True, ifTwoInput=False, outputM=False):
        ...
        # ========================================================================================
        # CHANGE 2: INITIALIZE THE NEW DENOISING MODULE
        # We add our new DenoisingResBlock as a component of the generator.
        # It will act as the final stage of the network.

        self.denoiser = DenoisingResBlock(channels=NUM_BANDS)
    ...
    def forward(self, inputs):
        ...
        # ========================================================================================
        # CHANGE 3: APPLY THE DENOISER IN THE FORWARD PASS
        # The raw reconstructed LST (l1) is now passed through our new denoiser
        # before being returned. This is the new final step of the generator.
        # ========================================================================================
